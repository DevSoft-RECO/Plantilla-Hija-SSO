...
<template>
    <div class="div3 bg-white/10 backdrop-blur-md flex flex-col overflow-hidden rounded-2xl shadow-2xl relative transition-all border border-verde-cope/30">
        ...
        <!-- Interactive Area -->
        <div v-if="selectedSolicitud" class="p-2 bg-white/5 backdrop-blur-md border-t border-white/10 shrink-0">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex flex-wrap gap-1 overflow-x-auto pb-0.5 max-w-[60%] custom-scrollbar no-scrollbar items-center">
                         <div v-for="(file, idx) in form.evidencias" :key="idx"
                              class="bg-blue-500/20 text-blue-100 px-1.5 py-0.5 rounded-md text-[9px] flex items-center gap-1 border border-blue-500/30 shadow-sm">
                             <span class="max-w-[70px] truncate">{{ file.name }}</span>
                             <button @click="removeFile(idx)" class="hover:text-red-400 transition text-[8px]"><i class="fas fa-times"></i></button>
                         </div>
                    </div>
