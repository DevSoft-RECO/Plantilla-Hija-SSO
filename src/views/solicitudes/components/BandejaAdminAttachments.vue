...
<template>
    <div class="div4 bg-white dark:bg-gray-800 flex flex-col overflow-hidden rounded-2xl shadow-md relative transition-shadow hover:shadow-lg border border-gray-100 dark:border-gray-700">
        ...
        <div class="flex-1 overflow-y-auto p-1.5 custom-scrollbar">
            <div v-if="!selectedSolicitud" class="h-full flex flex-col justify-center items-center text-gray-400 dark:text-gray-600 p-2 text-center">
                <i class="fas fa-folder-open text-xl mb-1 opacity-50"></i>
                <p class="text-[9px] font-medium">Vacío</p>
            </div>
            <div v-else-if="loadingDetail" class="flex gap-2">
                <div v-for="i in 3" :key="i" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-1.5 flex flex-col items-center justify-center animate-pulse h-18 w-20">
                    <div class="h-6 w-6 bg-gray-200 dark:bg-gray-700 rounded mb-1"></div>
                    <div class="h-1.5 w-10 bg-gray-200 dark:bg-gray-700 rounded"></div>
                </div>
            </div>
            <div v-else-if="allFiles.length === 0" class="h-full flex flex-col justify-center items-center text-gray-400 italic text-[10px]">
                Sin archivos
            </div>
            <div v-else class="flex flex-row overflow-x-auto custom-scrollbar gap-3 pb-1 pt-0.5 px-0.5">
                <div v-for="(file, idx) in allFiles" :key="idx"
                     ...
                     class="shrink-0 w-20 bg-white dark:bg-gray-800 rounded-lg flex flex-col items-center justify-center text-center cursor-pointer shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all group h-18 relative overflow-hidden border border-gray-100 dark:border-gray-700">
                    ...
                    <div v-if="!isImage(file.name)" class="w-full h-full bg-gray-50 dark:bg-gray-700 flex items-center justify-center transition-transform group-hover:scale-105" style="color: var(--color-azul-cope)">
                        <div class="w-10 h-10 rounded-full bg-white dark:bg-gray-800 shadow-sm flex items-center justify-center border border-gray-100 dark:border-gray-700">
